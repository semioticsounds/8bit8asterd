...
public:
  ...
  ushort m_pitch;
  int m_attack, m_ampl, m_ampl_top, m_decay, m_sustain, m_release;
...
// 11 = Release and on          [6]
  ...
  if (m_chan == 1 || m_chan == 4 || m_chan == 7){
    psg.setEnvelope_B(fxp.envdecay, 9);
  }  
  else if (m_chan == 2 || m_chan == 5 || m_chan == 8){
    psg.setEnvelope_C(fxp.envdecay, 9);
  }
  else if (m_chan == 0 || m_chan == 3 || m_chan == 6){
    psg.setEnvelope_A(fxp.envdecay, 9);
  }
  Serial.print("CH: ");
